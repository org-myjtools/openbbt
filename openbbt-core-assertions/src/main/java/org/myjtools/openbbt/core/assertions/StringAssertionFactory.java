...
public class StringAssertionFactory extends AssertionFactoryAdapter<String> {
	...
	protected void fillSuppliers() {
		...
		suppliers.put(
				...
				it -> new AssertionAdapter(name, Matchers.not(Matchers.comparesEqualTo(it))));
		suppliers.put(
		...
		suppliers.put(
				...
				it -> new AssertionAdapter(name, Matchers.equalToIgnoringCase(it))
		);
		suppliers.put(
		...
		suppliers.put(
				...
				it -> new AssertionAdapter(name, Matchers.startsWith(it))
		);
		suppliers.put(
				ASSERTION_STRING_STARTS_WITH_IGNORE_CASE,
				it -> new AssertionAdapter(name, Matchers.startsWithIgnoringCase(it))
		);
		suppliers.put(
				ASSERTION_STRING_ENDS_WITH,
				it -> new AssertionAdapter(name, Matchers.endsWith(it))
		);
		suppliers.put(
				ASSERTION_STRING_ENDS_WITH_IGNORE_CASE,
				it -> new AssertionAdapter(name, Matchers.endsWithIgnoringCase(it))
		);
		suppliers.put(
				ASSERTION_STRING_CONTAINS,
				it -> new AssertionAdapter(name, Matchers.containsString(it))
		);
		suppliers.put(
				ASSERTION_STRING_CONTAINS_IGNORE_CASE,
				it -> new AssertionAdapter(name, Matchers.containsStringIgnoringCase(it))
		);
		suppliers.put(
				ASSERTION_STRING_NOT_EQUALS_IGNORE_CASE,
				it -> new AssertionAdapter(name, Matchers.not(Matchers.equalToIgnoringCase(it)))
		);
		suppliers.put(
		...
		suppliers.put(
				...
				it -> new AssertionAdapter(name, Matchers.not(Matchers.startsWith(it)))
		);
		suppliers.put(
				ASSERTION_STRING_NOT_STARTS_WITH_IGNORE_CASE,
				it -> new AssertionAdapter(name, Matchers.not(Matchers.startsWithIgnoringCase(it)))
		);
		suppliers.put(
				ASSERTION_STRING_NOT_ENDS_WITH,
				it -> new AssertionAdapter(name, Matchers.not(Matchers.endsWith(it)))
		);
		suppliers.put(
				ASSERTION_STRING_NOT_ENDS_WITH_IGNORE_CASE,
				it -> new AssertionAdapter(name, Matchers.not(Matchers.endsWithIgnoringCase(it)))
		);
		suppliers.put(
				ASSERTION_STRING_NOT_CONTAINS,
				it -> new AssertionAdapter(name, Matchers.not(Matchers.containsString(it)))
		);
		suppliers.put(
				ASSERTION_STRING_NOT_CONTAINS_IGNORE_CASE,
				it -> new AssertionAdapter(name, Matchers.not(Matchers.containsStringIgnoringCase(it)))
		);
	}
